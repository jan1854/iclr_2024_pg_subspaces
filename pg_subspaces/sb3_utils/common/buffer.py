...
class ReplayBufferDiffCheckpointer:
    ...
    def save(self) -> None:
        ...
        for field, name in fields:
            if rb.pos > first_pos_to_save:
                ...
                data[name] = np.concatenate((field[first_pos_to_save:rb.buffer_size], field[:rb.pos]))
    ...
    def load(self, timestep: Optional[int] = None) -> None:
        ...
        for checkpoint in checkpoints:
            ...
            assert (self.algorithm.replay_buffer.pos + data["observations"].shape[0]) % self.algorithm.replay_buffer.buffer_size == data["pos"]
    ...
    def _add_samples_to_buffer(self, observations: np.ndarray, actions: np.ndarray, rewards: np.ndarray, dones: np.ndarray, next_observations: np.ndarray) -> None:
        ...
        for rb_field, data_field in fields:
            rb_field[rb.pos:rb.pos + num_samples_buffer_end] = data_field[:num_samples_buffer_end]
    ...
    def _get_replay_buffer_checkpoints(self, step: Optional[int]) -> List[Tuple[int, Path]]:
        if step is None:
